...
def ссылка_в_указателе(отн, подпись, описание):
    """Статья должна попасть и в articles.html: в разметку и в видимый список.
    Раньше это делалось руками и четыре статьи так и не попали в указатель."""
    p = os.path.join(ПАПКА, "articles.html")
    g = io.open(p, encoding="utf-8").read()
    if отн in g:
    ...
    блок = ('    {\n'
            ...
            '     "url": "%sdocs/%s",\n'
            '     "description": "%s"\n'
            '    }\n   ]' % (подпись.replace('"', "'"), САЙТ, имя, описание.replace('"', "'")))
    хвост = '    }\n   ]'
    if хвост not in g:
        шаг("ВНИМАНИЕ: разметка указателя не разобрана, вписал только список")
    else:
        место = g.rindex(хвост)
        g = g[:место] + '    },\n' + блок + g[место + len(хвост):]

    пункт = ('      <li>\n'
             '        <h3><a href="%s">%s</a></h3>\n'
             '        <p>%s</p>\n'
             '      </li>\n' % (отн, подпись, описание))
    конец = g.rindex("    </ul>") if замена else g.index("    </ul>")
    g = g[:конец] + пункт + g[конец:]

    io.open(p, "w", encoding="utf-8").write(g)
    шаг("статья вписана в указатель")
...
if __name__ == "__main__":
    ...
    ссылка_в_указателе(отн, подпись, описание)
